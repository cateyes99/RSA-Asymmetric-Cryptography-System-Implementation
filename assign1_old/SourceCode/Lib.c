 ...
 * (not -3), and the remainder a[] = 3 * 2^return (not -1 * 2^return), because
 * -13 = -3 * 4 - 1 = -4 * 4 + 3.
 *
 ...
 * is changed to 2^return times as its original, and this is the same case for
 * the remainder container a[]. As a result, the remainder should be
 * a[]/2^return (i.e., a[]/d[]). c[] & d[] need be initialised to 0 before
 * calling.
 *                                         - Changed by XuZF
 */
int	divide1( int a[], int b[], int c[], int d[], int n, int m)
{
	...
	while (b[m-1] < (r/2))
	{
		...
		for (j=0; j <= n-2; j++)	// Because, n = n + 1 above!
			d[j] = 2 * d[j];
		for (j=0; j <= n-2; j++)
		{
			d[j+1] = d[j+1] + d[j]/r;
...
/*
 * a mod b => c[]. n and m is the same as in divide1( ). a[] & b[] haven't been
 * changed in this function. b[] must be positive (Refer to divide1( )'s
 * description), and c[] has been initialised in this function.
 */
void	Mod(int a[], int b[], int c[], int n, int m)
{
	int	tt[MaxElem], t1[MaxElem], t2[MaxElem];

	InitArray( tt, 0, 0, FixLen-1);
	InitArray( t1, 0, 0, FixLen-1);
	InitArray( t2, 0, 0, FixLen-1);
	InitArray( c, 0, 0, FixLen-1);

	ArrayA2B( a, tt, FixLen);
	ArrayA2B( b, t1, FixLen);
	divide1( tt, t1, c, t2, n, m);
	InitArray( t1, 0, 0, FixLen-1);
	InitArray( c, 0, 0, FixLen-1);
	divide1( tt, t2, c, t1, n, m);
}
...
{
    int y[MaxElem], mask, ii, jj, tt[MaxElem], tt1[MaxElem], tt2[MaxElem];
...
//    for (ii = 2 * nn - 1; ii >= 0 && e[ii] == 0; ii--)
    ...
    for ( ; ii >= 0; ii--)
    {
        ...
        for (jj = RadixBits - 1; jj >= 0; jj--)
        {
            ...
            InitArray( tt, 0, 0, FixLen-1);
			ArrayA2B( c, tt2, FixLen);	// Important
            divide1( y, tt2, res, tt, 2*nn, nn);
            InitArray( res, 0, 0, FixLen-1);
            InitArray( tt1, 0, 0, FixLen-1);    // Not necessary
            divide1( y, tt, res, tt1, 2*nn, nn);
...
//printf( "2. y[ ] = %lld\n", IntArrayValue( y, r, 2*nn));
                ...
                InitArray( res, 0, 0, FixLen-1);    // Computing res % c
                InitArray( tt, 0, 0, FixLen-1);
			    ArrayA2B( c, tt2, FixLen);
                divide1( y, tt2, res, tt, 2*nn, nn);
                InitArray( res, 0, 0, FixLen);
                InitArray( tt1, 0, 0, FixLen);    // Not necessary
                divide1( y, tt, res, tt1, 2*nn, nn);
 ...
 * funcs might not able to do this way, because Japcobi func has its own
 * characteristics.
 */
 ...
 * a[] mod b[] => a[], b[] isn't changed in this function
 */
...
//printf( "r1[ ] = %lld\n", IntArrayValue( r1, r, nn));
		...
		InitArray( tt, 0, 0, FixLen-1);
		ArrayA2B( r1, t1, FixLen);
		divide1( r0, t1, q1, tt, nn, nn);

		InitArray( t1, 0, 0, FixLen-1);
		InitArray( r2, 0, 0, FixLen-1);
		divide1( r0, tt, r2, t1, nn, nn);
